...
const initSearch = async () => {
  ...
  const controller = new Septima.Search.Controller([],{ 
    blankBehavior: "search",
    onSelect: selectHandler,
  })

  var historySearcher = new Septima.Search.HistorySearcher({
    controller,
    resultType: new Septima.Search.ResultType({
      id: 'historik',
      singular: 'Historik',
      plural: 'Historik',
    }),
    store: new Septima.Search.CookieResultStore({
      source: 'historik',
      sortMode: 'filter',
      maxLength: 20,
    }),
  ...
  const searchableComponentsJson = new Septima.Search.SearchableData({
    ...
    plural: "Komponenter",
    data: components,
    searchProperties: ["title", "description", "body"],
